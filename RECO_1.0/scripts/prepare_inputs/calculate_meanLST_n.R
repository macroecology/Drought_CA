...
tiles<- levels(factor(substr(tif,21,26)))
...
for (tile in tiles) {
  ...
  grid<- readGDAL(paste(workd,dates[1],"/LSTnight.2003.01.01.",tile,".gapfilled.tif",sep=""),silent=TRUE)
  ...
  for (date in dates) { 
    if (file.exists(paste(workd,date,"/LSTnight.",date,".",tile,".gapfilled.tif",sep=""))==TRUE) {
      LSTd<- readGDAL(paste(workd,date,"/LSTnight.",date,".",tile,".gapfilled.tif",sep=""),silent=TRUE)
  ...
  writeGDAL(LST.mean,paste(workd,"meanLST_n/LSTnightmean.2003.",tile,".tif",sep=""))
  writeGDAL(LST.count,paste(workd,"meanLST_n/LSTnight.na.count.2003.",tile,".tif",sep=""))
